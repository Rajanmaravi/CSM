...
{
    public class FeedbackDaoValidator:AbstractValidator<FeedbackDaoModel>
    {
        public FeedbackDaoValidator()
        {
            RuleFor(p => p.FeedbackCode)
                .Cascade(CascadeMode.StopOnFirstFailure)
                .NotEmpty().WithMessage("{PropertyName} should be not empty.")
                .Length(1, 100).WithMessage("{PropertyName} should not be empty and should not exceed 100 characters!");

            RuleFor(p => p.FeedbackName)
               .Cascade(CascadeMode.StopOnFirstFailure)
               .NotEmpty().WithMessage("{PropertyName} should be not empty.")
    ...
    public class FeedbackSearchValidator : AbstractValidator<FeedbackSearchModel>
    {
        public FeedbackSearchValidator()
        {
            RuleFor(p => p.FeedbackCode)
                .Cascade(CascadeMode.StopOnFirstFailure)
